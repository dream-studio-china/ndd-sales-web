...
│           ├── page.jsx
│           ├── page.module.css
│           ├── ChenpiIntro.jsx   # 陈皮引言组件
│           └── RipenessStages.jsx# 五段成熟刻度组件
├── public/
│   └── assets/
│       ├── sales-map-game*.webp/png # 高清底图（hd/通用/webp）
│       ├── nantan/               # nantan-hero.png + nantan-origin.html（经 rewrites 暴露）
│       └── regions/              # 16 个 hero 视效（chakeng/dadong/daze/dongjia/gujing/huicheng-chenpi/meijiang/muzhou/qibao/sanjiang/shadui/shuangshui/siqian/tianlu/tianma/yamen）
├── src/
│   ├── App.jsx                   # 核心：regions + mapGeometry + documentedRegionIds + Overview/RegionDetail
│   └── regionStories.js          # 15 产区风土叙事数据集（storyBySlug，新增 shuangshui/yamen 统一收口）
├── docs/
...
- 服务端组件，`generateStaticParams` 枚举 15 个 slug，`generateMetadata` 取 `summary`，`RegionStoryPage` 按 `variant/motif` 与 `colors` 主题渲染 hero/来处/风土/柑香/价值/入境/时令，`SeasonScale.jsx` 复用时令刻度。

### `app/regions/huicheng/page.jsx` / `public/assets/nantan/nantan-origin.html`

- `huicheng` 陈皮通识页聚焦五段成熟刻度，已拆分为 `ChenpiIntro.jsx`（引言/传统认识）与 `RipenessStages.jsx`（五段刻度与四季日历）；`nantan` 为静态 HTML 经 `next.config.mjs` rewrites 暴露；`shuangshui`/`yamen` 已由早期定制页收口至 `[slug]` 通用模板。
...
- 视效资源：`public/assets/regions/*.webp`（16 个 hero：含 `shuangshui`/`yamen` 已收口）+ `public/assets/nantan/nantan-hero.png`
- 模板：`[slug]` 通用模板承载 15 个产区（含双水/崖门），`huicheng` 单独承载陈皮知识（拆分为 `ChenpiIntro`/`RipenessStages`），`nantan` 为静态 HTML 过渡
