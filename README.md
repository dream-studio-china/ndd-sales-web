...
> 江门新会区销售战区可视化 — 基于 React + Vite 的移动端优先交互式地图
...
| React DOM | 19.0.0 | 渲染 |
| Vite | 6.4.3 | 构建与开发服务器 |
| 原生 CSS | — | `src/styles.css`，无框架依赖 |
| SVG / Canvas | — | 矢量区域 + Canvas 图片透明化处理 |

## 功能特性

- **完整无空隙地图**：所有区域共享同一坐标系，通过 `mapGeometry` 与 `mapOutline` 拼接为一张完整地图（`src/App.jsx:36`）。
